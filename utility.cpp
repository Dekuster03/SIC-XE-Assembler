/*This file contains the functions helpful for the functioning of the other files
The functions declared here rarely change
...
void readFirstNonWhiteSpace(string line, int &index, bool &status, string &data, bool readTillEnd = false)
{
  ...
  while (index < line.length() && checkWhiteSpace(line[index]))
  { 
    //Increase the index to pass all the whitespaces
    index++;
  }
}
...
void readByteOperand(string line, int &index, bool &status, string &data)
{
  ...
  if (line[index] == 'C')
  {
    ...
    { 
      //Copy all the data until the next identifier regardless of whitespace
      data += line[index];
      index++;
    }
    data += identifier;
  ...
  while (index < line.length() && checkWhiteSpace(line[index]))
  { 
    //Increase the index to pass all the whitespace
    index++;
  }
}
