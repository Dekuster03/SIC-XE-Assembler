/*This is my code for pass1*/
#include "utility.cpp" /*This file conatins all the important headers*/
#include "tables.cpp"

using namespace std;

/*The variable to keep persisted*/
string fileName;
...
void handle_LTORG(string &litPrefix, int &lineNumberDelta, int lineNumber, int &LOCCTR, int &lastDeltaLOCCTR, int currentBlockNumber)
{
  ...
  for (auto const &it : LITTAB)
  {
    ...
    if (litAddress != "?")
    {
      /*Pass as the address is already assigned*/
    }
    else
    {
      lineNumber += 5;
...
void evaluateExpression(string expression, bool &relative, string &tempOperand, int lineNumber, ofstream &errorFile, bool &error_flag)
{
  ...
  for (int i = 0; i < expression.length();)
  {
    ...
    {
      //Checking the existence of the operand
    ...
    {
      //Checking the legallity of the expressions
...
void pass1()
{
  ...
  while (checkCommentLine(fileLine))
  {
    ...
    getline(sourceFile, fileLine); //This reads the first line of the input
    lineNumber += 5;
    index = 0;
  }
  ...
  if (opcode == "START")
  {
    ...
    writeToFile(intermediateFile, writeData); // This writes the file to intermediate file

    getline(sourceFile, fileLine); //This reads the next line of the input
    lineNumber += 5;
    index = 0;
    readFirstNonWhiteSpace(fileLine, index, statusCode, label);  //This parses the label
    readFirstNonWhiteSpace(fileLine, index, statusCode, opcode); //This parses the OPCODE
  }
  else
  {
    startAddress = 0;
  ...
  while (opcode != "END")
  {
    ...
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (!checkCommentLine(fileLine))
      {
        ...
        {
          //This searches for the opcode in OPTAB
        ...
        else if (opcode == "ORG")
        {
          ...
          {
            bool relative;
            //This sets the error_flag to false
            error_flag = false;
            ...
            error_flag = false; //This resets the error_flag
          }
        }
        else if (opcode == "USE")
        {
        ...
        else if (opcode == "EQU")
        {
          ...
          else
          {
            ...
            //This is the code for reading the whole operand
            evaluateExpression(operand, relative, tempOperand, lineNumber, errorFile, error_flag);
          }
      ...
      BLOCKS[currentBlockName].LOCCTR = intToStringHex(LOCCTR); //This updates LOCCTR of block after every instruction
      getline(sourceFile, fileLine);                            //To read the next line
    ...
    if (opcode != "END")
    {
      ...
      getline(sourceFile, fileLine); //This reads the next line
      lineNumber += 5;
