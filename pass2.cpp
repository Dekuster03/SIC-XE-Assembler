...
string createObjectCodeFormat34()
{
  ...
  { 
    ...
    { 
      ...
      /*This processes the immediate value*/
    ...
    else
    {
      ...
      /*This processes the Immediate symbol value*/
      if (halfBytes == 5)
      { 
        /*If it is of format 4*/
        objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += '1';
        objcode += intToStringHex(operandAddress, halfBytes);

        /*Add the modification record here*/
      ...
      /*Use direct addressing with no program-counter or base*/
  ...
  else if (getFlagFormat(operand) == '@')
  {
    ...
    { 
      //No base or program-counter based addressing in format 4
      objcode = intToStringHex(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
      objcode += '1';
      objcode += intToStringHex(operandAddress, halfBytes);
  ...
  { 
    ...
    if (tempOperand == "*")
    {
      ...
      /*Add the modification record for value created by operand `*` */
...
void writeEndRecord(bool write = true)
{
  ...
  { 
    //Make an operand on end firstExecutableAddress
...
void pass2()
{
  ...
  getline(intermediateFile, tempBuffer); // Discard the heading line
  ...
  while (isComment)
  { 
    //Handle with all previous comments
  ...
  while (opcode != "END")
  {
    while (opcode != "END")
    {
      if (!isComment)
      {
        ...
        else if (opcode == "NOBASE")
        {
          if (nobase)
          { 
            //Check if the assembler was using base addressing
    ...
    if (!isComment)
    {
      ...
      writeToFile(objectFile, modificationRecord, false); //Write the modification record
      writeEndRecord(true);                               //Write the end record
